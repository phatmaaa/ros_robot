...
class RosRobot:
    ...
    def __init__(self, robot_controller):
        #The constructor method of the `RosRobot` class. 
        ...
        self.set_TCP('camera_color_optical_frame')
    ...
    def setup_tf(self):
        # This method sets up transformations between
        #  different frames of reference using the `kinematics` module. It defines the static 
        # transformations between different coordinate frames.
    ...
    def change_ref_vel_cb(self, req):
        # A callback function that changes the 
        # reference velocity of the robot. It receives a value through a service
        #  call and updates the velocity attribute accordingly.
        self.vel = req.value
    ...
    def change_ref_acc_cb(self, req):
        ...
        # it changes the reference acceleration of the robot.
        self.acc = req.value
    ...
    def moveRobot_cb(self, req, slow=False):
        ...
        #A callback function to move the robot to a specific 
        # pose. It receives the target pose through a service call, sets the TCP (Tool Center Point),
        #  and calls the `move_to_pose` method to perform the movement.
    ...
    def moveRobotRelative_cb(self, req, slow=False):
        ...
        # but it moves the robot relative to the current pose.
        self.set_TCP(req.frame)
    ...
    def moveTCP_cb(self, req):
        ...
        #A callback function to adjust the position
        # of the TCP (Tool Center Point) by a specific distance. It receives the adjustment values through a service 
        # call and calls the `adjust_pose_callback` method
        self.set_TCP(req.frame)
    ...
    def move_robot_callback(self, twist_msg):
        ...
        #The method sets the velocity command for the robot based on the received message.
        #Callback to set specific velocity command to robot
        #This method sets the velocity command for the robot
        # based on the received message. It receives a Twist message containing linear and angular velocity components and 
        # converts it to the corresponding TCP velocity vector.
    ...
    def move_pose_callback(self, pose_msg):
        #This method is a callback function that is called when a target pose is received for the robot.
        #It takes a Pose message as input, which contains the position and orientation of the target pose.
        #The method moves the robot to the target pose.
    ...
    def move_to_pose(self, pose_msg, slow=False, relative_frame='ur_base'):
        #A method that moves the robot to a target pose.
        #  It calculates the transformation matrix from the current pose to the target pose and calls the `move_TCP`
        #  method to perform the movement.
    ...
    def move_to_pose_list(self, pose_msg_list, slow=False):
        #Similar to the previous method, but it accepts a 
        #list of target poses and moves the robot sequentially.
    ...
    def adjust_pose_callback(self, Pose_msg):
        #A callback function to adjust the pose of
        #  the active TCP (Tool Center Point). It receives the desired pose adjustment through 
        # a service call and calls the `move_frame` method.
    ...
    def move_frame(self, frame, transformation_matrix):
        #A method that moves a specific frame of reference by
        # applying a transformation matrix to it. It receives the frame name and the desired 
        # transformation matrix and calls the `move_TCP` method.
    ...
    def move_TCP(self, desired_transformation, slow=False):
        #A method that moves the robot's TCP (Tool Center Point)
        # to a desired transformation matrix. It receives the desired transformation matrix and calls the appropriate method 
        # from the robot controller to perform the movement.
    ...
    def angle_callback_z(self, target_angle_msg):
        #A callback function to rotate the robot's end effector around the 
        # Z-axis of the TCP. It receives the target angle through a message and 
        # calls the `move_frame` method to perform the rotation.
    ...
    def run_node(self):
        #The main loop function that continuously updates the poses and velocities of the robot.
        # It publishes the poses and velocities to corresponding ROS topics.
    ...
    def run_controller(self):
        #A separate thread function that continuously sends
        # velocity commands to the robot controller based on the received velocity vector.
        while not rospy.is_shutdown():
    ...
    def update_poses(self):
        # This method updates the current poses and velocities of the robot. It retrieves the robot's pose, camera's pose,
        #  and other related information from the robot controller and stores them in the corresponding attributes of the class.
    ...
    def move_PF_to_cam(self, msg):
        # A method to move the pressure force/torque 
        # sensor to the location of the camera.
    ...
    def set_TCP(self, target_TCP):
        #A method to set the active TCP (Tool Center Point). It receives
        #the target TCP name and updates the corresponding attribute.
    ...
    def set_TCP_cb(self, req):
        #A callback function to set the active TCP through a service call. 
        #It receives the target TCP name and calls the `set_TCP` method.
        self.set_TCP(req.frame)
    ...
    def fire_drill_cb(self, activate_flag):
        # A callback function to perform a drilling operation.
        # It receives an activation flag and performs the drilling operation.
    ...
    def pick_item(self, req): #TODO: remove
        ...
        #A method to perform a top-down grasp of an item.
